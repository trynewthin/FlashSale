...
function Resolve-Migrate {
    ...
    throw "migrate executable not found after install"
}
...
try {
    foreach ($db in $dbs) {
        ...
        Write-Host "migrate up -> $name"
        & $migrate -path $path -database $dsn up
        if ($LASTEXITCODE -ne 0) {
            throw "migrate up failed for $name with exit code $LASTEXITCODE"
        }
    }
} finally {
    Pop-Location
...
Write-Host "migrate-up finished"
