...
本文档用于核对 HTTP 路由、Gateway Handler 与 RPC 方法是否对齐。它是架构参考页，不承担接口使用说明。
...
| `POST /api/v1/admin/seckill/activities/{activity_id}/items` | `SeckillAdminHandler.CreateActivityItem` | `SeckillRpc.UpsertActivityItem` |
...
| --- | --- | --- |
| `GET /api/v1/admin/dashboard/stats` | `DashboardHandler.GetStats` | `UserRpc.ListUsers` + `ProductRpc.ListProductsAdmin` + `OrderRpc.ListOrdersAdmin` + `SeckillRpc.ListActivitiesAdmin` |

## 3. 对齐结论

- 当前网关路由与 RPC 方法保持一一对应，没有明显断链。
- 统计类接口在网关层做并发聚合，不强行映射到单一 RPC。
- 管理侧鉴权和领域权限应始终视为网关与下游共同承担的约束。
