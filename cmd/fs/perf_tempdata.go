// perf_tempdata 为压测提供“临时数据创建/回收”能力，保证测试隔离与可重复执行。
package main
...
func setupPerfTempData(repoRoot, scenario string, perfArgs []string, options perfControlOptions) (*perfTempData, error) {
	...
	adminToken, err := loginAdminForPerf(adminBaseURL, repoRoot)
	if err != nil {
		return nil, err
	}
	...
	defer func() {
		...
		if cleanupErr := cleanupPerfTempData(temp); cleanupErr != nil {
			fmt.Fprintf(os.Stderr, "WARN: rollback temp data failed: %v\n", cleanupErr)
		}
	}()
	uniqueSuffix := strconv.FormatInt(now.UnixNano(), 10)

	productID, err := createTempProductForPerf(adminBaseURL, adminToken, uniqueSuffix)
	if err != nil {
		finalErr = err
		return nil, err
	}
	...
	activityID, err := createTempActivityForPerf(adminBaseURL, adminToken, now, uniqueSuffix)
	if err != nil {
		finalErr = err
		return nil, err
	}
	...
	itemID, err := createTempActivityItemForPerf(adminBaseURL, adminToken, activityID, productID)
	if err != nil {
		finalErr = err
		return nil, err
	}
	...
	if err := publishTempActivityForPerf(adminBaseURL, adminToken, activityID); err != nil {
		finalErr = err
		return nil, err
	}
...
func cleanupPerfTempData(temp *perfTempData) error {
	...
	if temp.ActivityID > 0 {
		if err := deleteTempActivityForPerf(temp.AdminBaseURL, temp.AdminToken, temp.ActivityID); err != nil {
			errs = append(errs, "delete activity: "+err.Error())
		}
	}
	if temp.ProductID > 0 {
		if err := deleteTempProductForPerf(temp.AdminBaseURL, temp.AdminToken, temp.ProductID); err != nil {
...
// mergePerfArgsWithTempData 将临时资源覆盖到本次压测参数中。
func mergePerfArgsWithTempData(perfArgs []string, scenario string, temp *perfTempData) []string {
	if temp == nil {
		return perfArgs
	}
	out := stripPerfFlags(perfArgs, "activity-id", "item-id", "token", "token-file")
	out = append(out, "-activity-id", strconv.FormatInt(temp.ActivityID, 10))
	out = append(out, "-item-id", strconv.FormatInt(temp.ItemID, 10))
	if requiresPurchaseToken(scenario) && strings.TrimSpace(temp.TokenFile) != "" {
		out = append(out, "-token-file", temp.TokenFile)
	}
	return out
}
...
func loginAdminForPerf(adminBaseURL, repoRoot string) (string, error) {
	type cred struct {
		Username string
		Password string
	}
	candidates := make([]cred, 0, 4)
	add := func(username, password string) {
		...
		candidates = append(candidates, cred{Username: username, Password: password})
	}
...
func adminLoginOnce(adminBaseURL, username, password string) (string, error) {
	...
	statusCode, envelope, err := callPerfAPIEnvelope(defaultPerfHTTPClient(), "POST", requestURL, "", map[string]any{
		"username": username,
		"password": password,
	})
	if err != nil {
...
func createTempProductForPerf(adminBaseURL, adminToken, suffix string) (int64, error) {
	requestURL := strings.TrimRight(adminBaseURL, "/") + "/api/v1/admin/products"
	statusCode, envelope, err := callPerfAPIEnvelope(defaultPerfHTTPClient(), "POST", requestURL, adminToken, map[string]any{
...
func deleteTempProductForPerf(adminBaseURL, adminToken string, productID int64) error {
	requestURL := fmt.Sprintf("%s/api/v1/admin/products/%d", strings.TrimRight(adminBaseURL, "/"), productID)
	statusCode, envelope, err := callPerfAPIEnvelope(defaultPerfHTTPClient(), "DELETE", requestURL, adminToken, nil)
	if err != nil {
		return err
	}
	if statusCode < 200 || statusCode >= 300 || strings.TrimSpace(envelope.Code) != "OK" {
...
func createTempActivityForPerf(adminBaseURL, adminToken string, now time.Time, suffix string) (int64, error) {
...
func deleteTempActivityForPerf(adminBaseURL, adminToken string, activityID int64) error {
...
func createTempActivityItemForPerf(adminBaseURL, adminToken string, activityID, productID int64) (int64, error) {
...
func publishTempActivityForPerf(adminBaseURL, adminToken string, activityID int64) error {
...
func createTempUsersAndTokensForPerf(userBaseURL, repoRoot string, userCount int, uniqueSuffix string) ([]int64, string, error) {
	if userCount <= 0 {
		return nil, "", nil
	}
	...
	for index := 0; index < userCount; index++ {
		phone := buildPerfTempPhone(uniqueSuffix, index)
		nickname := fmt.Sprintf("perf_u_%d", index+1)
		sourceIP := buildPerfTempSourceIP(index)
		if err := registerPerfTempUserWithRetry(userBaseURL, phone, password, nickname, sourceIP); err != nil {
			return nil, "", err
		}
		token, userID, err := loginPerfTempUserWithRetry(userBaseURL, phone, password, sourceIP)
		if err != nil {
			return nil, "", err
		}
...
func deleteTempUserForPerf(adminBaseURL, adminToken string, userID int64) error {
...
func registerPerfTempUserWithRetry(userBaseURL, phone, password, nickname, sourceIP string) error {
	maxAttempts := envIntFromEnv("FLASHSALE_PERF_USER_AUTH_MAX_ATTEMPTS", 8)
	if maxAttempts < 1 {
		maxAttempts = 1
	}
	var lastErr error
	for attempt := 1; attempt <= maxAttempts; attempt++ {
		retryable, err := registerPerfTempUser(userBaseURL, phone, password, nickname, sourceIP)
		if err == nil {
			return nil
		}
		lastErr = err
		...
		time.Sleep(perfTempUserRetryBackoff(attempt))
	}
	return lastErr
}

func registerPerfTempUser(userBaseURL, phone, password, nickname, sourceIP string) (bool, error) {
	requestURL := strings.TrimRight(userBaseURL, "/") + "/api/v1/user/register"
	statusCode, envelope, err := callPerfAPIEnvelopeWithHeaders(defaultPerfHTTPClient(), "POST", requestURL, "", map[string]any{
		"phone":    phone,
		"password": password,
		"nickname": nickname,
	}, perfTempUserSourceHeaders(sourceIP))
	if err != nil {
		return true, err
	}
	if statusCode >= 200 && statusCode < 300 && strings.TrimSpace(envelope.Code) == "OK" {
	...
	return shouldRetryPerfTempUserRequest(statusCode, envelope, nil), err
}

func loginPerfTempUserWithRetry(userBaseURL, phone, password, sourceIP string) (string, int64, error) {
	maxAttempts := envIntFromEnv("FLASHSALE_PERF_USER_AUTH_MAX_ATTEMPTS", 8)
	if maxAttempts < 1 {
		maxAttempts = 1
	}
	var lastErr error
	for attempt := 1; attempt <= maxAttempts; attempt++ {
		token, userID, retryable, err := loginPerfTempUser(userBaseURL, phone, password, sourceIP)
		if err == nil {
...
func loginPerfTempUser(userBaseURL, phone, password, sourceIP string) (string, int64, bool, error) {
	...
	if statusCode < 200 || statusCode >= 300 || strings.TrimSpace(envelope.Code) != "OK" {
		...
		return "", 0, shouldRetryPerfTempUserRequest(statusCode, envelope, nil), err
	}
	var payload struct {
		AccessToken string      `json:"access_token"`
...
func buildPerfTempPhone(uniqueSuffix string, index int) string {
...
func buildPerfTempSourceIP(index int) string {
	// 使用不同来源键避开网关注册/登录按来源限流；仅用于压测临时数据准备。
	if index < 0 {
		index = -index
	}
...
func shouldRetryPerfTempUserRequest(statusCode int, envelope perfAPIEnvelope, callErr error) bool {
	...
	if code == "SYS_BAD_REQUEST" && strings.Contains(message, "请求过于频繁") {
		return true
	}
	return false
}
...
// resolveTempUserCount 计算购买类场景临时用户数量。
func resolveTempUserCount(scenario string, configured int, perfArgs []string) int {
	if scenario == perfScenarioIdempotency {
		return 1
	}
	if configured > 0 {
		return configured
	}
	concurrency := envIntFromEnv("FLASHSALE_PRESSURE_CONCURRENCY", 100)
	if value, found, err := findPerfFlagValue(perfArgs, "concurrency"); err == nil && found {
