...
run_scope_action() {
    ...
    case "$scope" in
        all|proxy)
            if [[ "$action" == "build" || "$action" == "run" || "$action" == "restart" || "$action" == "apply" ]]; then
    ...
    case "${scope}:${action}" in
        ...
        all:apply) run_action "全部 / 更新" apply_all_scope_sequence ;;
        all:stop)
        ...
        infra:apply) run_action "基础设施 / 更新" apply_infra_sequence ;;
        ...
        backend:apply) run_action "后端 / 更新" apply_backend_sequence ;;
        ...
        proxy:apply) run_action "代理 / 更新" apply_proxy_sequence ;;
        ...
        ops:apply) run_action "Ops / 更新" apply_ops_sequence ;;
        ...
        observability:apply) run_action "可观测 / 更新" apply_observability_sequence ;;
