...
const adminSeckillManagementDomain = "seckill_management"

func authorizeAdminSeckillDomain(ctx context.Context) (int64, error) {
	...
	if !hasDomain(claims.Domains, adminSeckillManagementDomain) {
...
func authorizeUser(ctx context.Context, targetUserID int64) error {
	...
	if uid != targetUserID {
		return errorx.New(errorx.CodeAuthForbidden, "无权限访问该用户资源")
	}
	return nil
}
