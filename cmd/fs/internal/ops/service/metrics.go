...
var MetricAllowlist = []model.MetricDef{
	...
	{
		Name:   "rpc_error_rate",
		Query:  `(sum(rate(rpc_server_requests_code_total{code=~"Internal|Unavailable|DeadlineExceeded|Unknown|DataLoss"}[1m])) or vector(0)) / sum(rate(rpc_server_requests_code_total[1m])) * 100`,
		Unit:   "%",
