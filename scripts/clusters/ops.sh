...
#
# 注意：ops-control 的 Dockerfile 基于 flashsale-backend:local，
# 因此必须在 backend.sh build 完成后才能构建。
#
# 用法：
#   ./scripts/clusters/ops.sh            # 构建镜像 + 启动
#   ./scripts/clusters/ops.sh build      # 只构建镜像
#   ./scripts/clusters/ops.sh up         # 只重启（镜像已存在）
#   ./scripts/clusters/ops.sh hot        # 热更新：本地交叉编译 + docker cp（最快，~15s）
...
_build_frontend() {
    step "FRONTEND" "Building ops frontend"
...
case "$CMD" in
    build)
        _build_frontend
        step "BUILD" "Building ops-control image (requires flashsale-backend:local)"
    ...
    all)
        _build_frontend
        step "BUILD" "Building ops-control image"
        dc_build "$@" ops-control
