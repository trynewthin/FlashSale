...
func TestDecodeJSONRejectTrailingData(t *testing.T) {
	req := httptest.NewRequest(http.MethodPost, "/api/v1/admin/test", strings.NewReader(`{"a":1}{"b":2}`))
	var out struct {
		A int `json:"a"`
	}
...
func TestDecodeJSONAcceptSingleObject(t *testing.T) {
	req := httptest.NewRequest(http.MethodPost, "/api/v1/admin/test", strings.NewReader(`{"a":1}`))
	var out struct {
		A int `json:"a"`
	}
	if err := decodeJSON(req, &out); err != nil {
